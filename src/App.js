...
class Gallery extends Component {
  ...
  delete = index => {
    const dataWithItemGone = this.state.gifyArray.filter(
      item => this.state.gifyArray.indexOf(item) !== index
    );
  ...
  componentDidMount() {
    axios
      .get(
        `https://api.giphy.com/v1/gifs/trending?api_key=${process.env.REACT_APP_GIFY_API_KEY}`
      )
      .then(response => {
        ...
        data.forEach(el => {
          ...
          o.width = parseInt(el.images.downsized_large.width) * 1.5;
          o.height = parseInt(el.images.downsized_large.height) * 1.5;
  ...
  render() {
    ...
    return (
      <div id="rotatingImg" className="active-image">
        ...
        {restartButton}
        <GalleryThumb
          array={this.state.gifyArray}
          action={this.handleClick}
          removeAction={this.delete}
        />
      </div>
    );
  }
